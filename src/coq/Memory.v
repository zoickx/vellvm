...
CoFixpoint memD {A} (memory:mtype) (d:Obs A) : Obs A :=
  match d with
    ...
    | Eff (Alloca t k)  => Tau (memD (memory ++ [undef])%list (k (DVALUE_Addr t (pred (List.length memory)))))
...
Fixpoint MemDFin {A} (memory:mtype) (d:Obs A) (steps:nat) : option mtype :=
  ...
  | S x =>
    ...
    | Eff (Alloca t k)  => MemDFin (memory ++ [undef])%list (k (DVALUE_Addr t (pred (List.length memory)))) x
